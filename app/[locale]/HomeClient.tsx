  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--bg)' }}>
      ...
      >
          ...
          <div style={{ display: 'flex', animation: 'marquee 100s linear infinite', width: 'max-content' }}>
            {TECH_MARQUEE_ROW1.map((item, i) => (
              <span
                key={i}
                style={{
                  padding: '0 16px', fontSize: '10.5px', fontWeight: 700,
                  letterSpacing: '0.22em', whiteSpace: 'nowrap', textTransform: 'uppercase',
                  fontFamily: "'Courier New', monospace", color: 'var(--accent)', textShadow: 'var(--tech-glow-accent)',
                }}
              >
        ...
        <div style={{ height: '1px', margin: '0 56px', background: 'var(--tech-strip-sep)' }} />
            ...
            {TECH_MARQUEE_ROW2.map((item, i) => (
              <span
                ...
                style={{
                  padding: '0 16px', fontSize: '10.5px', fontWeight: 700,
                  letterSpacing: '0.22em', whiteSpace: 'nowrap', textTransform: 'uppercase',
                  fontFamily: "'Courier New', monospace", color: 'var(--accent-purple)', textShadow: 'var(--tech-glow-purple)',
                }}
              >
                <span style={{ opacity: 0.4, marginRight: '5px' }}>{'<'}</span>
                {item}
      ...
      <div className="max-w-6xl mx-auto px-4 py-14 phone:py-8">
        ...
        <div className="grid pad-v:grid-cols-2 pad:grid-cols-2 pc:grid-cols-2 gap-6 mb-12 phone:mb-8">
          <SectionCard className="p-8 phone:p-6 flex flex-col">
            ...
            <div className="flex gap-3 flex-wrap">
              <GlowButton href={resumeLink} download style={{ padding: '8px 16px', fontSize: '14px' }}>
              ...
              <GlowButton href={`/${locale}/projects`} variant="outline" style={{ padding: '8px 16px', fontSize: '14px' }}>
                {t('PROJECTS')}
              </GlowButton>
            </div>
          </SectionCard>
          ...
          <div className="grid grid-cols-3 gap-4 phone:gap-3">
            {stats.map((stat) => (
              ...
              >
                <span className="font-black leading-none mb-2" style={{ fontSize: 'clamp(32px, 4.5vw, 60px)', color: 'var(--accent)' }}>
                  {stat.value}
                </span>
                <span className="uppercase tracking-widest" style={{ fontSize: 'clamp(9px, 0.85vw, 11px)', color: 'var(--text-muted)' }}>
                  {stat.label}
                </span>
              </SectionCard>
            ))}
        ...
        {/* ── Featured Work ─────────────────────────────────── */}
        <div className="mb-12">
          <SectionHeader title={t('MY_PROJECTS')} linkHref={`/${locale}/projects`} linkLabel={t('VIEW_ALL')} />

          <div className="grid pad-v:grid-cols-2 pad:grid-cols-3 pc:grid-cols-3 gap-5">
            {featuredProjects.map((proj) => (
              <SectionCard key={proj.title} className="group overflow-hidden flex flex-col transition-all duration-300 hover-accent-border">
                <div className="relative h-44 overflow-hidden shrink-0">
                  ...
                  <span
                    ...
                    style={{ background: 'var(--accent-dim)', color: 'var(--accent)', border: '1px solid var(--accent-glow)', fontFamily: "'Courier New', monospace" }}
                  >
                    {proj.company}
                  </span>
                </div>
                ...
                <div className="p-5 flex flex-col flex-1">
                  <h3 className="text-[var(--text)] font-bold text-sm mb-2 leading-snug">{proj.title}</h3>
                  <p className="text-[var(--text-muted)] text-xs leading-relaxed mb-4 flex-1 line-clamp-2">{proj.desc}</p>

                  <div className="flex flex-wrap gap-1.5 mb-4">
                    {proj.tags.map((tag) => (
                      <span key={tag} className="text-[9px] px-2 py-0.5 rounded font-bold uppercase tracking-wider"
                  ...
                  {proj.url ? (
                    <GlowButton href={proj.url} external className="w-fit" style={{ padding: '5px 12px', fontSize: '11px', opacity: 0.85 }}>
                    ...
                    <GlowButton href={`/${locale}${proj.routeLink}`} className="w-fit" style={{ padding: '5px 12px', fontSize: '11px' }}>
                      Try It →
                    </GlowButton>
                  ) : (
                    <GlowButton href={`/${locale}/projects`} variant="outline" className="w-fit" style={{ padding: '5px 12px', fontSize: '11px' }}>
                      {t('VIEW_DETAILS')}
                    </GlowButton>
                  )}
                </div>
          ...
          <div className="flex flex-col pad:flex-row pc:flex-row">
            ...
            <div className="pad:w-7/12 pc:w-7/12 p-8 pad:p-12 pc:p-12 flex items-center">
              <div>
                ...
                <blockquote className="text-xl phone:text-base text-[var(--text-muted)] italic mb-5 pl-4"
                  style={{ borderLeft: '2px solid var(--accent)' }}>
                  &ldquo;{keepLearningArray[1]}&rdquo;
                </blockquote>
                <p className="text-right text-sm text-[var(--text-dim)]">— {keepLearningArray[2]}</p>
              </div>
            </div>
          </div>
        </SectionCard>
